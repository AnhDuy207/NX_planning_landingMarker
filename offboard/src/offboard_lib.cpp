...
void OffboardControl::inputSetpoint() {
    ...
    if (mode == '1') {
        ...
        std::printf("- Choose (3): Mission with ENU setpoint & Yaw\n");
        std::printf("- Choose (4): Mission with Planner\n");
        std::printf("- Choose (5): Mission with Planner and Landing at marker\n");
        std::printf("- Choose (6): Mission with ENU setpoint & Yaw & Landing at setpoint\n");
...
void OffboardControl::plannerAndLandingFlight() {
    ...
    while (ros::ok()) {
        ...
        // std::cout << "x = " << abs(setpoint.pose.position.x - current_odom_.pose.pose.position.x) << std::endl;
        // std::cout << "y = " << abs(setpoint.pose.position.y - current_odom_.pose.pose.position.y) << std::endl;
        ...
        if (first_target_reached) {
            ...
            hovering(targetTransfer(current_odom_.pose.pose.position.x, current_odom_.pose.pose.position.y, current_odom_.pose.pose.position.z, degreeOf(yaw_)), 2.0);
            break;
        }
        ros::spinOnce();
        rate.sleep();
...
void OffboardControl::plannerFlight()
{
    ...
    mav_trajectory_generation::drawMavTrajectory(trajectory_, distance, "map", &markers);int count = end(target_array_.data) - begin(target_array_.data);
    // num_of_enu_target_ = (count)/3;
    // for(int i=0; i<num_of_enu_target_; i++){
    //     x_target_[i] = target_array_.data[i*3+0];
    //     y_target_[i] = target_array_.data[i*3+1];
...
//DuyNguyen
// void OffboardControl::deliveryHover(geometry_msgs::PoseStamped setpoint, double unpack_time)
// {
//     ros::Rate rate(10.0);
//     //bool land_reached = false;
//     bool target_reached = false;
...
//         target_enu_pose_ = targetTransfer(current_odom_.pose.pose.position.x + components_vel_.x, current_odom_.pose.pose.position.y + components_vel_.y, current_odom_.pose.pose.position.z + components_vel_.z, setpoint.pose.orientation);
//         std::cout << "Yaw= " << setpoint.pose.orientation << std::endl;
//         target_enu_pose_.header.stamp = ros::Time::now();
//         setpoint_pose_pub_.publish(target_enu_pose_);
...
//             if (current_state_.system_status == 3)
//             {
//                 hovering(targetTransfer(current_odom_.pose.pose.position.x, current_odom_.pose.pose.position.y, current_odom_.pose.pose.position.z, setpoint.pose.orientation), unpack_time);
//                 // TODO: unpack service
//             }
//             else
//             {
